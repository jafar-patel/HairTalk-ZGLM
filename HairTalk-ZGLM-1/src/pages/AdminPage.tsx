...
function DashboardTab({ showToast }: { showToast: (msg: string) => void }) {
  const { content, resetToDefaults, supabaseConnected, loading } = useSite();
  ...
  return (
    <div>
      ...
      <div className={`mb-6 p-4 rounded-xl border ${supabaseConnected ? 'bg-emerald-50 border-emerald-200' : loading ? 'bg-amber-50 border-amber-200' : 'bg-red-50 border-red-200'}`}>
        <div className="flex items-center gap-3">
          <div className={`w-3 h-3 rounded-full ${supabaseConnected ? 'bg-emerald-500' : loading ? 'bg-amber-500 animate-pulse' : 'bg-red-500'}`} />
          <div>
            <p className="text-sm font-medium">{supabaseConnected ? 'Supabase Connected' : loading ? 'Connecting to Supabase...' : 'Supabase Not Connected'}</p>
            <p className="text-xs text-gray-500 mt-0.5">{supabaseConnected ? 'All changes sync to the cloud automatically.' : loading ? 'Please wait...' : 'Data is saved locally only. Add your Supabase anon key in src/lib/supabase.ts'}</p>
          </div>
        </div>
      </div>
